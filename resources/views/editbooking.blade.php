...
<form action="/editbooking" method="POST" class="editbooking" style="width:82%;margin-left:5vh;display:flex; align-items:center;">
    <br>
    <fieldset style="padding:30px; border:4px solid  #7C9CBA; width:80%;">
        <legend><h3>&nbsp;&nbsp;Edit Booking</h3></legend>
        @csrf<input type="hidden" name="id" value="{{ isset($data->id) ? $data->id : '' }}">
        ...
        @foreach ($services as $service)
            <div class="form-check">
                <input class="form-check-input" type="checkbox" name="services[]" id="service{{ $service->id }}" value="{{ $service->id }}" {{ in_array($service->id, $data->services->pluck('id')->toArray()) ? 'checked' : '' }}>
                <label class="form-check-label" for="service{{ $service->id }}">{{ $service->name }}</label>
            </div>
        @endforeach
        <br>&nbsp;&nbsp;<b>Event Schedule: </b><input type="date" name="event_schedule" value="{{ isset($data->event_schedule) ? $data->event_schedule : '' }}"><br>
